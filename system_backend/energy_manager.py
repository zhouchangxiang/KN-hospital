...
from database.constant import myHours,mydays,mymonths
from common.MESLogger import MESLogger
from common.BSFramwork import AlchemyEncoder
logger = MESLogger('../equipment_backend/logs', 'log')
from common.common_cuid import insertSyslog
...
db_session = Session()
energycuid = Blueprint('energycuid', __name__, template_folder='templates')

@energycuid.route('/energytrendtu', methods=['GET', 'POST'])
def energytrendtu():
    '''
    能耗趋势图
    :return:
...
@energycuid.route('/energyanalysis', methods=['GET', 'POST'])
def energyanalysis():
    '''
    能耗分析
    :return:
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if CompareTime:
                ...
                tag_str = "SUM(IncremenValue) AS A_ACR_10"
                ...
                for i in re:
                    dict_i[i["CollectionHour"]] = \
                        round(float(0 if i["A_ACR_10"] == None else i["A_ACR_10"]), 2)
                ...
                row1_list = []
                row1_list.append({"电表": "总耗量", "今日": round(curr_A_ACR_10_count, 2), "对比日": round(comp_A_ACR_10_count, 2)})
...
@energycuid.route('/energyselectbytime', methods=['GET', 'POST'])
...
@energycuid.route('/makecoolanalysis', methods=['GET', 'POST'])
...
@energycuid.route('/makecoolselectbytime', methods=['GET', 'POST'])
...
# @energycuid.route('/selectEquipmentEfficiencyTree')#组织结构
...
#             return json.dumps([{"status": "Error：" + str(e)}], cls=AlchemyEncoder, ensure_ascii=False)
#
# @energycuid.route('/selectrundetailbyequipmentcode', methods=['GET', 'POST'])
